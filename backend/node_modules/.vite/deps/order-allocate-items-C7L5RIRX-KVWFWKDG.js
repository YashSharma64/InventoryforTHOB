...
// node_modules/@medusajs/dashboard/dist/order-allocate-items-C7L5RIRX.mjs
...
function defaultAllocations(items) {
  ...
  items.forEach((item) => {
    var _a, _b;
    const hasInventoryKit = checkInventoryKit(item);
    ret[hasInventoryKit ? `${item.id}-` : `${item.id}-${(_a = item.variant) == null ? void 0 : _a.inventory[0].id}`] = "";
    if (hasInventoryKit) {
      (_b = item.variant) == null ? void 0 : _b.inventory.forEach((i) => {
        ret[`${item.id}-${i.id}`] = "";
      });
    }
  });
...
//# sourceMappingURL=order-allocate-items-C7L5RIRX-KVWFWKDG.js.map
